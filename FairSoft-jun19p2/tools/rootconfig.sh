#!/bin/bash
   ...
   if [ "$build_python" = "yes" ];
   then
      PYTHONBUILD="-Dpython=ON -DPYTHON_INCLUDE_DIR=/usr/include/python3.6 -DPYTHON_LIBRARY=/usr/lib/python3.6/config-3.6m-x86_64-linux-gnu/libpython3.6.so -DPYTHON_EXECUTABLE=/usr/bin/python3"
   else
